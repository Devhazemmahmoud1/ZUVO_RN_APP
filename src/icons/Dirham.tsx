// components/DirhamLogo.tsx
import React from 'react';
import Svg, { Path, SvgProps } from 'react-native-svg';

type Props = SvgProps & {
  /** Height/width in px; keeps the original aspect ratio */
  size?: number;
  /** Fill color for the logo path */
  color?: string;
};

const DirhamLogo: React.FC<Props> = ({ size = 24, color = '#000', ...svgProps }) => {
  return (
    <Svg
      width={size}
      height={(size * 299.91) / 344.84} // preserve aspect ratio from the viewBox
